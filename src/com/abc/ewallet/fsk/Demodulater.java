...
public class Demodulater {
	...
	public boolean demodulateFMData(float[] buffer) {
	...
	public boolean demodulateFMDataFindHeader(float[] buffer) {
		try {
			...
			for (int i = start; i < end - phase * 9; i++) {
				...
				double b = (double) (buffer[i + phase + 30] - buffer[i]) / phase;
				if (b > angleThreshold) {
...
//							phase =  ( peaks[2].point - peaks[0].point )/4;
	...
	public boolean demodulateFMData2(float[] buffer) {
		write( "Demo: " );
		for (int i = (nextPoint - frameCounter * bufferSize - 1); i < bufferSize; i += phase) {
			float max = Math.max(buffer[i-1], buffer[i]);
			if ( max > threshold ) {
	...
	private int getNearMaxIndex( float[] buffer, int i, int len ) {
		float max = 0;
		int index = 0;
